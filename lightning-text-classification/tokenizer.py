...
class Tokenizer(TextEncoder):
    ...
    @property
    def vocab(self) -> list:
        """Returns: list: list of tokens in the dictionary"""
        return self.tokenizer.vocab

    @property
    def vocab_size(self) -> int:
        """Returns: int: Number of tokens in the dictionary"""
        return len(self.itos)

    def encode(self, sequence:str) -> torch.Tensor:
        """Encodes a 'sequence'/
        :param sequences: String 'sequence' to encode.
        :return : torch.Tensor with Encoding of the 'sequence'
        """
        sequence = TextEncoder.encode(self, sequence)
        return self.tokenizer(sequence, return_tenors='pt')['input_ids'][0]
